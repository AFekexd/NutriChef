...
export const authenticate = async (req, res, next) => {
    try {
        ...
        console.log("[authenticate] Token received (first 20 chars):", token.substring(0, 20) + "...");
        // Verify token
        ...
        if (session.user.lockedUntil && session.user.lockedUntil > new Date()) {
            console.log("[authenticate] Account is locked until:", session.user.lockedUntil);
            res.status(403).json({
        ...
        if (session.ipAddress !== currentIp) {
            console.warn(`IP address mismatch for user ${payload.userId}: session IP ${session.ipAddress}, current IP ${currentIp}`);
            // You could add stricter IP validation here if needed
            // For now, we'll just log it and continue
        }
        // Attach user to request
        req.user = payload;
        console.log("[authenticate] Authentication successful, proceeding to route handler");
        next();
    }
    catch (error) {
