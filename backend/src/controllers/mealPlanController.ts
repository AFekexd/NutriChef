...
export const updateMealPlan = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const userId = req.user?.userId;
    const { date, mealType, recipeIds, targetCalories, targetMacros } =
      req.body;
    ...
    if (targetMacros !== undefined) updateData.targetMacros = targetMacros;
...
export const getWeeklySummary = async (req: Request, res: Response) => {
  try {
    ...
    mealPlans.forEach((plan: any) => {
      plan.mealPlanRecipes.forEach((mpr: any) => {
