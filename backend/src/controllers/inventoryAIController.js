...
export const uploadInventoryImage = async (req, res) => {
    try {
        ...
        const detectionResult = await visionAI.detectIngredients(optimizedPath, language);
        ...
        console.log("[uploadInventoryImage] Upload record created:", uploadRecord.uploadId);
...
export const confirmDetectedItems = async (req, res) => {
    try {
        ...
        for (const item of items) {
            ...
            if (!ingredient) {
                console.log("[confirmDetectedItems] Creating new ingredient:", item.name);
        ...
        console.log("[confirmDetectedItems] Successfully created items:", createdItems.length);
        res.json({
...
export const getInventoryAnalytics = async (req, res) => {
    try {
        ...
        console.log("[getInventoryAnalytics] Total items:", itemsWithIngredients.length);
        const categoryCount = {};
        ...
        const response = {
            ...
            aiDetectedCount,
            aiDetectionPercentage: itemsWithIngredients.length > 0
...
export const updateConsumption = async (req, res) => {
    try {
        ...
        if (logs.length >= 2) {
            const totalConsumed = logs.reduce((sum, log) => sum + log.quantityConsumed, 0);
            const firstLog = logs[0];
            const lastLog = logs[logs.length - 1];
            if (firstLog && lastLog) {
                const daysDiff = (lastLog.dateTime.getTime() - firstLog.dateTime.getTime()) /
                    (1000 * 60 * 60 * 24);
                if (daysDiff > 0) {
...
//# sourceMappingURL=inventoryAIController.js.map