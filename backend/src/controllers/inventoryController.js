...
export const getMyInventoryItems = async (req, res) => {
    try {
        if (!req.user) {
...
export const createInventoryItem = async (req, res) => {
    try {
        const { userId, ingredientId, quantity, unit, expiryDate, consumptionRate, } = req.body;
...
export const getMyExpiringItems = async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: "Authentication required" });
        }
...
export const addManualInventoryItem = async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: "Authentication required" });
        }
        const { ingredientName, quantity, unit, location, expiryDate, category } = req.body;
        // Validation
